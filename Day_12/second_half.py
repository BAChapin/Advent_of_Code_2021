...
def part2_filter(possible_vertex, current_path):
    lower = [vertex for vertex in current_path if vertex.islower()]
    is_small_duplicate = len(lower) != len(set(lower))
    return part1_filter(possible_vertex, current_path) or (possible_vertex != 'start' and not is_small_duplicate)
...
def process():
    ...
    paths = find_all_paths(graph, part2_filter)
